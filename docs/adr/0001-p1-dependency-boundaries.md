...
```text
CLI request -> SQLite-backed pending approval -> Tauri desktop approve/reject -> CLI status/audit
```
...
| CLI parsing | `clap` | Mature derive-based parser with help and subcommand support | Do not hand-roll argument parsing or shell completion | Command semantics such as `request`, `status`, `queue`, `audit`, `approve`, `reject` |
...
- CLI output that exposes stable, checker-friendly observable fields
...
This decision keeps P1 focused on proving the approval and audit backbone while preserving room to grow into later provider and policy phases. It also gives checker and future contributors a concrete line between acceptable domain implementation and avoidable reinvention.
